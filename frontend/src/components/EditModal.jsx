...
const EditModal = ({ isOpen, onClose, item, onSave, onDelete, previousEvent, availableDates = [], currentDate }) => {
    ...
    const commonPlaces = [
        '中部国際空港 (セントレア)', '那覇空港', '名古屋駅', '東京駅', '新宿駅',
        '京都駅', '大阪駅', '博多駅', '札幌駅', '高山駅', '下呂駅', '飛騨古川駅',
        '沖縄美ら海水族館', '首里城', '国際通り', '万座毛', 'ホテル日航アリビラ',
        'リザンシーパークホテル谷茶ベイ', 'ANAインターコンチネンタル万座ビーチリゾート'
    ];
        ...
        <div className="fixed inset-0 z-modal flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 w-full max-w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh] sm:max-h-[85vh] animate-slide-up-spring overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 overflow-y-auto overflow-x-hidden space-y-4 flex-1">
                        ...
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-3 space-y-3">
                            ...
                            <div className="relative">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1.5 block">
                                    出発地 {autoFrom && formData.from === autoFrom && <span className="text-indigo-500">(自動)</span>}
                                </label>
                            ...
                            {/* To */}
                            <div className="relative">
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1.5 block">到着地</label>
                                <div className="relative flex items-center gap-2">
                                    <input
                                        type="text"
                                        value={formData.to || ''}
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder={`未入力時: ${formData.from || '出発地'} → ${formData.to || '到着地'}`}
                                />
                                <p className="text-[10px] text-gray-400 mt-1 px-1">カード表示時のタイトルをカスタマイズできます</p>
                            </div>
                        </div>
                    )}
                    ...
                    {!isTransport && (
                        <div className="relative">
                            <label className="text-[10px] font-bold text-gray-400 uppercase mb-1.5 block">名称</label>
                            <div className="relative flex items-center gap-2">
                    ...
                    {/* Details/Memo */}
                    <div>
                        <label className="text-[10px] font-bold text-gray-400 uppercase mb-1.5 block">メモ</label>
                        <textarea
                        ...
                        onClick={() => {
                            ...
                            if (isTransport) {
                                ...
                                if (formData.customName && formData.customName.trim()) {
                                    ...
                                    dataToSave.name = `${formData.from} → ${formData.to}`;
                                } else if (formData.to) {
