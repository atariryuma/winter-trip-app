        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/50" onClick={onClose}>
            <div className="bg-white w-full max-w-md rounded-t-3xl shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 border-t border-gray-100 flex gap-3">
                    {item && onDelete && (
                        <button onClick={() => onDelete(item.id)} className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-100"><Trash2 size={20} /></button>
                    )}
                    <button onClick={() => onSave(formData)} className="flex-1 bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 flex items-center justify-center gap-2">
                        <Save size={18} /> 保存する
                    </button>
                </div>
            </div>
...
export default function TravelApp() {
    const [itinerary, setItinerary] = useState(() => {
        const saved = sessionStorage.getItem('trip_data_v5');
    ...
    useEffect(() => {
        sessionStorage.setItem('trip_data_v5', JSON.stringify(itinerary));
    }, [itinerary]);
    ...
    return (
        <div className="max-w-md mx-auto min-h-screen pb-24 bg-gray-50 shadow-2xl overflow-hidden relative">

            {/* Header Gradient */}
            <div className="bg-gradient-to-br from-blue-600 to-indigo-800 p-6 text-white pt-10 pb-16 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-20">
                    <Plane size={120} />
                </div>
                <div className="relative z-10">
                    <h1 className="text-2xl font-bold mb-1">Winter Journey</h1>
                ...
                <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
                    {itinerary.map(day => (
                        <button
                            ...
                            className={`flex-shrink-0 flex flex-col items-center justify-center w-16 h-20 rounded-2xl shadow-sm transition-all duration-300 ${selectedDayId === day.id
                    ...
                    <div className="relative pl-4 space-y-6">
                        ...
                        {sortedEvents.map(event => (
                            <div key={event.id} className="relative pl-6">
                                {/* Timeline Dot */}
                                <div className={`absolute left-0 top-3 w-8 h-8 rounded-full border-4 border-gray-50 bg-white flex items-center justify-center shadow-sm z-10 -translate-x-1/2 ${event.type === 'stay' ? 'ring-2 ring-indigo-100' : ''}`}>
                                ...
                                <div
                                    ...
                                    className={`rounded-2xl p-4 shadow-sm border border-gray-100 transition bg-white ${event.type === 'stay' ? 'bg-indigo-50/50 border-indigo-100' : ''} ${isEditMode ? 'cursor-pointer hover:shadow-md hover:border-blue-300' : ''}`}
                                >
                                    {/* Time & Badge */}
                                    ...
                                    {/* Title */}
                                    <h3 className="font-bold text-gray-800 text-lg mb-1">{event.name}</h3>
                                    ...
                                    {(event.description || event.details) && (
                                        <div className="mt-3 pt-3 border-t border-dashed border-gray-200 text-sm text-gray-600">
                                    ...
                                    {event.bookingRef && (
                                        <div
                                            ...
                                            className="mt-3 bg-white border border-gray-200 rounded-lg p-2 flex items-center justify-between cursor-pointer active:bg-gray-100 group"
                                        >
            ...
            {/* Bottom Nav */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-gray-200 px-6 py-3 flex justify-around items-center z-20 max-w-md mx-auto">
